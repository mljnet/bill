...
// API endpoint untuk collector stats
router.get('/api/collector-stats/:collectorId', async (req, res) => {
    try {
        const collectorId = req.params.collectorId;

        // Get collector payments statistics
        const stats = await new Promise((resolve) => {
            db.get(`
                SELECT
                    COUNT(cp.id) as totalPayments,
                    SUM(cp.payment_amount) as totalAmount,
                    SUM(cp.commission_amount) as totalCommission,
                    COUNT(DISTINCT cp.customer_id) as totalCustomers
                FROM collector_payments cp
                WHERE cp.collector_id = ?
            `, [collectorId], (err, row) => {
                if (err) {
                    console.error('❌ Error getting collector stats:', err);
                    resolve({
                        totalPayments: 0,
                        totalAmount: 0,
                        totalCommission: 0,
        ...
        console.log('📊 Collector Stats API - Data loaded:', stats);
        ...
        console.error('❌ Error in collector stats API:', error);
        res.status(500).json({
            success: false,
            error: error.message
        });
